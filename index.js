...
client.on('message', message =>{
    ...
    if(command === 'ping'){
    ...
    } else if (command === 'str_eth_price') {
        message.channel.send('Checking prices...');
        require('dotenv').config();
        ...
        let message1 = `
                +-------------+----------------+---------------+----------------------------+---------------------------+
 | Input token | Output token |  Input amnt   |         Uniswap return         |  Kyber expected return  |
+-------------+----------------+---------------+----------------------------+---------------------------+
 |      `+inputTokenSymbol+`        |        `+outputTokenSymbol+`          |            `+web3.utils.fromWei(inputAmount, 'Ether')+`            |`+ web3.utils.fromWei(uniswapResult, 'Ether')+`|`+web3.utils.fromWei(kyberResult.expectedRate, 'Ether')+`|
+-------------+----------------+---------------+----------------------------+---------------------------+
            ` ;
...
const fluxInstance = new flux.default("testnet", "api.flux.market", KeyStore);
console.log(fluxInstance);
fluxInstance.connect("fluxprotocol-phase-point-two").then(() => {
    console.log("connected to:", fluxInstance);
});
*/
